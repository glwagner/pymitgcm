...
for i in range(nt):
    ...
    T = vars['T'][:, setup.ny//2, :].squeeze()

    im[0] = axs[0].contourf(xg, z, u, cmap='RdBu_r', levels=ulevs)
    axs[0].contour(x, z, T, ntlevs, colors='grey', linewidths=lw)

    im[1] = axs[1].contourf(x, z, w,  cmap='RdBu_r', levels=wlevs)
    axs[1].contour(x, z, T, ntlevs, colors='grey', linewidths=lw)
    ...
    message = "$t = {:.1e}$ s".format(t)
    axs[0].text(0.0, 1.03, message, transform=axs[0].transAxes)

    cb[0].update_normal(im[0])
    cb[1].update_normal(im[1])
